 ...
 * @extends  WP_Widget
 * @version  1.0.0
 * @package  Sx_Social_Icons/Abstracts
 * @category Widgets
...
abstract class TG_Widget extends WP_Widget {
	...
	public function get_cached_widget( $args ) {
		...
		$cache = wp_cache_get( apply_filters( 'sx_social_icons_cached_widget_id', $this->widget_id ), 'widget' );
	...
	public function cache_widget( $args, $content ) {
		wp_cache_set( apply_filters( 'sx_social_icons_cached_widget_id', $this->widget_id ), array( $args['widget_id'] => $content ), 'widget' );
	...
	public function flush_widget_cache() {
		wp_cache_delete( apply_filters( 'sx_social_icons_cached_widget_id', $this->widget_id ), 'widget' );
	}
	...
	public function update( $new_instance, $old_instance ) {
		...
		foreach ( $this->settings as $key => $setting ) {
			 ...
			 */
			$instance[ $key ] = apply_filters( 'sx_social_icons_widget_settings_sanitize_option', $instance[ $key ], $new_instance, $key, $setting );
		}
	...
	public function form( $instance ) {
		...
		foreach ( $this->settings as $key => $setting ) {
			...
			switch ( $setting['type'] ) {
				...
				default :
					do_action( 'sx_social_icons_widget_field_' . $setting['type'], $key, $value, $setting, $instance );
				break;
			}
		}
	}
